...
if [ -d $1 ]; then
  ...
  echo 'Iniciando a instalação nginx + php7.0 + mariadb + phpmyadmin'
...
echo "Instalando php 7.0 para" $1

docker run --name php70.$1 --network=intranet --restart always -l traefik.enable=false -v $PWD/html:/var/www/html -d php:7.0-fpm-stretch

docker exec -it php70.$1 sh -c 'apt-get update && apt-get install -y zlib1g-dev libc-client-dev libkrb5-dev libpng-dev && rm -r /var/lib/apt/lists/*'
docker exec -it php70.$1 sh -c 'docker-php-ext-configure imap --with-kerberos --with-imap-ssl && docker-php-ext-install imap zip mysqli mbstring gd'
...
server {
    ...
    location ~ \.php$ {
        ...
        fastcgi_pass $(docker ps -aqf 'name=php70.'$1):9000;
...
docker restart php70.$1 nginx.$1
