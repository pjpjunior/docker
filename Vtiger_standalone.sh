...
docker run --name nginx.$1 --network=intranet -p 80:80 -v $PWD/html:/var/www/html -v $PWD/default.conf:/etc/nginx/conf.d/default.conf:ro -v $PWD/error.log:/var/log/nginx/error.log -d nginx
...
docker run --name phpmyadmin.$1 -p 8080:80 --network=intranet -d -e PMA_HOST=mariadb.$1  phpmyadmin/phpmyadmin
