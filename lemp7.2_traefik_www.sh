...
docker run --name nginx.$1 --network=intranet --restart unless-stopped -l traefik.root.port=80 -l traefik.root.frontend.rule=Host:$1 -l traefik.www.port=80 -l traefik.www.frontend.rule=Host:www.$1 -l traefik.docker.network=web -l traefik.enable=true -v $PWD/html:/var/www/html -v $PWD/default.conf:/etc/nginx/conf.d/default.conf -v $PWD/error.log:/var/log/nginx/error.log -d nginx && docker network connect web nginx.$1
